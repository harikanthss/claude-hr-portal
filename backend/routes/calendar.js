...
router.get('/events', authenticateToken, (req, res) => { try { res.json(db.prepare('SELECT * FROM calendar_events ORDER BY date ASC').all()); } catch(err){res.status(500).json({error:err.message});} });
router.post('/events', authenticateToken, (req, res) => { try { const{title,date,endDate,type,color,description}=req.body; if(!title||!date) return res.status(400).json({error:'Title and date required'}); const id=genId('ev'); db.prepare('INSERT INTO calendar_events (id,title,date,endDate,type,color,description,createdBy) VALUES (?,?,?,?,?,?,?,?)').run(id,title,date,endDate||null,type||'meeting',color||'#3b82f6',description||'',req.user.id); res.status(201).json(db.prepare('SELECT * FROM calendar_events WHERE id=?').get(id)); } catch(err){res.status(500).json({error:err.message});} });
router.delete('/events/:id', authenticateToken, (req, res) => { try { db.prepare('DELETE FROM calendar_events WHERE id=?').run(req.params.id); res.json({message:'Deleted'}); } catch(err){res.status(500).json({error:err.message});} });
