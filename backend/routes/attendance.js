...
router.post('/check-in', authenticateToken, (req, res) => {
  try {
    ...
    else db.prepare('INSERT INTO attendance_records (id,employeeId,date,checkIn,status) VALUES (?,?,?,?,?)').run(genId('att'),req.user.id,today,checkIn,'present');
...
router.post('/check-out', authenticateToken, (req, res) => {
  try {
    ...
    const hours = Math.max(0, parseInt(checkOut)-parseInt(record.checkIn));
